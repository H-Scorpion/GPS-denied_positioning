...
def two_stage(distances_to_anchors, anchor_positions):
    ...
    for j in range(z_candidate.shape[0]):
        cost = 0
        for i in range(anchor_positions.shape[0]):
            d = (tag_pos[0]- anchor_positions[i][0])**2 + (tag_pos[1]- anchor_positions[i][1])**2 + (z_candidate[j] - anchor_positions[i][2])**2 
...
def costfun_method(distances_to_anchors,anchor_positions):
    ...
    getting better height estimation (all in meters). 
    ...
    """
    distances_to_anchors = np.array(distances_to_anchors)
    ...
    for i in range(stage_two_result.shape[0]):
        candi_pos.append(np.linalg.inv(rtx).dot(stage_two_result[i].T) + Anchor_mean)
...
if __name__ == '__main__':
    ...
    distances_to_anchors = [3.11,3.23,3.43,3.37]
    anchor_positions = [[0, 0, 0.0],
                        [0.288, 5.005, 0.0],
                        [1.974, 5.149, 0.0],
                        [2.245, 0, 0.0]]
    costfun_method(distances_to_anchors,anchor_positions)
    # anchor_num = 4
    # anchor_positions = [[0, 0, 0.0],
    #                     [0.288, 5.005, 0.0],
    #                     [1.974, 5.149, 0.0],
